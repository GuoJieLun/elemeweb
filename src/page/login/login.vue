<template>
  <div class="loginContainer">
    <header-top :head-title="loginWay? '登录' : '密码登录' " goBack=true>
    ...
    <alert-tip :alertText="alertText" v-if="showAlert" :showHide="showAlert" @closeTip="closeTip"></alert-tip>
  </div>
</template>
<script>
  ...
  import {mobileCode, accountLogin, sendLogin,getcaptchas, checkExsis, sendMobile} from '../../service/getData'
  export default{
    data(){
      return {
        ...
        validate_token: null,//获取短信返回的验证值,登录时需要
        computedTime: 0,//倒数计时
        serInfo: null, //获取到的用户信息
        ...
        if (this.rightPhoneNumber) {
          ...
          this.timer = setInterval(() => {
            ...
            if (this.computedTime == 0) {
              clearInterval(this.computedTime);
            }
          }, 1000);
          //判断用户是否存在
